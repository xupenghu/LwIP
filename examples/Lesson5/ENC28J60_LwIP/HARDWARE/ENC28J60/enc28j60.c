...
void enc28j60Init(unsigned char* macaddr)
{
    ...
    do
    {
        res = enc28j60Read(ESTAT) ;
    }while(!(res & ESTAT_CLKRDY)); //    
...
//元。
    ...
    res = enc28j60Read(MAADR5);
    res = enc28j60Read(MAADR4);
    res = enc28j60Read(MAADR3);
    res = enc28j60Read(MAADR2);
    res = enc28j60Read(MAADR1);
    res = enc28j60Read(MAADR0);
